...
-- 테이블에 추가한 데이터를 실제 데이터베이스에 반영 한다.
...
INSERT INTO MEMBER (ID, PW, NAME, DELYN, GENDER, AGE)
            VALUES ('ID', 'PW', '홍홍홍', 'Y', 'M', 30);

-- 오류
-- AGE는 0보다 큰 값이 입력되어야 하고
-- GENDER는 M,F만 입력가능
-- ID는 중복 될 수 없다
INSERT INTO MEMBER (ID, PW, NAME, DELYN, GENDER, AGE)
            VALUES ('ID', 'PW', '홍홍홍', 'Y', 'A', -20);
...
CREATE TABLE TB_BOOK(
    ...
    TITLE   VARCHAR2(100)   CONSTRAINT  TB_BOOK_TITLE           NOT NULL,
    AUTHOR  VARCHAR2(30)    CONSTRAINT  TB_BOOK_AUTHOR          NOT NULL,
    PRICE   NUMBER          CONSTRAINT  TB_BOOK_PRICE           CHECK(PRICE>0),
    RENTYN  CHAR(1)         DEFAULT     'N'
                            CONSTRAINT  TB_BOOK_RENTYN          CHECK(RENTYN IN('Y','N')),
    PUB_NO  NUMBER
);
...
INSERT INTO TB_BOOK(NO, TITLE, AUTHOR, PRICE, RENTYN, PUB_NO)
            VALUES(2,'BOOK1','AUTHOR1',10000, 'N', 123450);

-- 코드값을 생성 할 때 자릿수를 맞추기 위해서 함수를 사용할 수 있다.
...
SELECT SEQ_TB_BOOK.NEXTVAL, SEQ_TB_BOOK.CURRVAL FROM DUAL;
SELECT SEQ_TB_BOOK.CURRVAL FROM DUAL;
-- 시퀀스 삭제
...
INSERT INTO TB_BOOK(NO, TITLE, AUTHOR, PRICE, RENTYN, PUB_NO)
            VALUES('B' || LPAD(SEQ_TB_BOOK.NEXTVAL,5,'0'),'BOOK1',
                                            'AUTHOR1',10000, 'N', 123450);
...
CREATE TABLE TB_PUB(
    ...
    , PUB_NAME  VARCHAR2(60)    CONSTRAINT  TB_PUB_NAME     NOT NULL
    , PHONE     NUMBER
    , DELYN     CHAR(1)         DEFAULT 'N'                 CONSTRAINT  TB_PUB_DELYN
                                CHECK (DELYN IN ('Y','N'))
    , REGDATE   DATE            DEFAULT     SYSDATE
);

--모든 컬럼에 데이터를 입력할 경우 컬럼명을 생략 할 수 있다.
INSERT INTO TB_PUB VALUES ('P'||LPAD(SEQ_TB_PUB.NEXTVAL, 5, '0'),'출판사명1',0000000000,'N',SYSDATE);

INSERT INTO TB_PUB (PUB_NO, PUB_NAME, PHONE)
            VALUES ('P'||LPAD(SEQ_TB_PUB.NEXTVAL, 5, '0'),'출판사명1',0000000000);

CREATE SEQUENCE SEQ_TB_PUB;

SELECT * FROM TB_PUB;

SELECT SEQ_TB_PUB.NEXTVAL FROM DUAL;
-- SEQ_TB_PUB.NEXTVAL를 5자리로 만들고 남은 공간을 0으로 채운다.
...
-- 외래키제약조건을 추가 할 때 컬럼의 이름이 같지 않아도 된다.
CREATE TABLE TB_RENT(
    RENT_NO         CHAR(6)     PRIMARY KEY
    , RENT_MEM_NO   CHAR(6)     REFERENCES  MEMBER (ID)
...
INSERT INTO TB_RENT     (RENT_NO, RENT_MEM_NO, RENT_BOOK_NO)
            VALUES      ('R'||LPAD(SEQ_TB_RENT.NEXTVAL,5,'0'), 'M00000', 'B00000');
...
-- 제약조건이름 FOREIGN KEY (컬럼명) REFERNCES 테이블명 (컬럼명)
-- 테이블 레벨
CREATE TABLE MEMBER(
    ID VARCHAR2(20) NOT NULL
    , PW VARCHAR2(20) CONSTRAINT NN_MEMBER_PW NOT NULL
...
-- 자식테이블에서 입력할 때 오류가 발생 할 수 있다.
-- 부모테이블에 있는 데이터를 삭제 할 때 오류가 발생 할 수 있다.
-- 자식이 참조하고 있는 데이터는 삭제 불가 (기본값) -> 옵션을 주어서 삭제할 수 있다.
-- 옵션 :   1. 자식이 참조하고 있는 부모데이터를 NULL 로 업데이트
--         2.자식행을 함께 삭제
-- 부모테이블을 참조하고 있는경우 부모테이블의 입력된 데이터만 입력 가능
-- 무결성 : 데이터의 정확성과 일관성이 유지되고, 데이터에 결손과 부정함이 없음을 보증하는 것
...
-- 외래키 제약조건에 의해 TB_PUB테이블의 PUB_NO에 등록된 값만 입력 할 수 있다.
INSERT INTO TB_BOOK VALUES ('B00003', '제목', '작가', 20000, 'N', 'P00025');
-- 부모테이블에 입력된 값이 아니면 오류 발생
INSERT INTO TB_BOOK VALUES ('B00002', '제목', '작가', 20000, 'N', 'P00001');

-- DELETE문이나 UPDATE문을 사용 할 경우
-- 조건을 주지 않으면 테이블의 모든 데이터를 삭제
-- 건수를 확인하고 실행!
-- 만약 건수가 잘못되었다 싶으면 롤백!
ROLLBACK;
SELECT * FROM TB_BOOK;
...
INSERT INTO TB_BOOK VALUES ('B00001', '제목', '작가', 20000, 'N', 'P00025');

SELECT * FROM TB_RENT;
SELECT * FROM MEMBER;
SELECT * FROM TB_BOOK;

INSERT INTO TB_RENT (RENT_NO, RENT_BOOK_NO, RENT_MEM_NO) VALUES ('R00001', 'B00001', 'ID');
